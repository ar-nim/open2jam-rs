...
fn parse_m30(data: &[u8]) -> Result<SampleMap, OjmError> {
    ...
    while offset + SAMPLE_HEADER_SIZE <= data.len() {
        ...
        if sample_size == 0 || sample_size > MAX_ALLOWED_SAMPLE_SIZE || offset + sample_size > data.len()
        {
            break;
        }
...
fn parse_omc(data: &[u8], decrypt: bool) -> Result<SampleMap, OjmError> {
    ...
    while offset + OGG_HEADER_SIZE <= filesize {
        ...
        if sample_size == 0 || sample_size > MAX_ALLOWED_SAMPLE_SIZE || offset + sample_size > filesize
        {
            sample_id += 1;
            continue;
        }
