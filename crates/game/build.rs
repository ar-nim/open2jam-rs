...
//!
//! Automatically downloads Inter (Latin) and Noto Sans CJK SC (CJK) at build time
//! if they aren't already present in `crates/game/assets/`.
...
fn main() {
    ...
    if !assets_dir.join("NotoSansCJKsc-Regular.otf").exists() {
        if let Err(e) = download_noto_sans_sc(&assets_dir) {
            println!("cargo:warning=Failed to download Noto Sans CJK font: {}", e);
        }
    }
}
